...
export function MyBlogs(){
    ...
    const fetchData = async ()=>{
        try{
            const response = await axios.get('http://localhost:3000/api/blog/my-blogs', {
                params: {
                    token : token,
                },
            ...
            if(response.status === 201){
                ...
                if(response.data.user_id==response.data.author_id){
                    setEditable(true);
                }
            }
            else{
        ...
        <nav className="font-bold text-md flex bg-zinc-900 h-20 rounded-b text-white">
            ...
            <div className="flex flex-row justify-between mx-10 p-2">
                ...
                currentBlogs.map((blog) => (
                    ...
                    <Blog blog={blog} link={`/editblog/${blog._id}`}  editable={editable}/>
                    </div>
                ))
                ) : (
